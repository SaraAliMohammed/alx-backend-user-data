...
def reset_password_token(email: str) -> str:
    """It tests requesting a password reset."""
    pw_url = "{}/reset_password".format(BASE_URL)
    body = {'email': email}
    res = requests.post(pw_url, data=body)
    assert res.status_code == 200
    assert "email" in res.json()
...
def update_password(email: str, reset_token: str, new_password: str) -> None:
    """It tests updating a user's password."""
    pw_url = "{}/reset_password".format(BASE_URL)
    body = {
        'email': email,
    ...
    res = requests.put(pw_url, data=body)
    assert res.status_code == 200
